...
export function assertIsolatedStaging({ apps, productionApps = [], resources, organizations }) {
    ...
    const stagingOrganization = String(organizations?.staging ?? "").trim().toLowerCase();
    const productionOrganization = String(organizations?.production ?? "").trim().toLowerCase();
    if (!stagingOrganization || !productionOrganization) {
        throw new Error("Staging and production Fly organization identifiers are required.");
    }
    if (stagingOrganization === productionOrganization) {
        throw new Error("The staging Fly organization must be separate from production.");
    }
