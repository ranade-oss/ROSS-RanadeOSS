...
inject_failure_and_rollback() {
  ...
  if ! grep -Eqi 'health check|health checks|unhealthy|service is not listening|connection refused' \
    "$ARTIFACT_DIR/commands/web-forced-failure.log"; then
    echo "Invalid-port deployment failed for an unexpected reason; expected a Fly health-check failure." >&2
    exit "$deploy_status"
  fi
