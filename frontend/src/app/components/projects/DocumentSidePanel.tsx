...
export function DocumentSidePanel({
  ...
  function handleResizeMouseDown(e: React.MouseEvent<HTMLDivElement>) {
    ...
    const handleMouseMove = (event: MouseEvent) => {
      ...
      setDataColumnWidth(
        Math.min(maxDataWidth, Math.max(MIN_DATA_COLUMN_WIDTH, nextWidth)),
      );
    };
      ...
      <div className="flex min-h-11 shrink-0 items-center justify-between gap-3 px-4 py-2 md:h-11 md:py-0">
        ...
        <div className="flex shrink-0 items-center gap-1.5">
          ...
          <button
            ...
            className="inline-flex h-7 items-center gap-1.5 rounded-md px-2 text-xs font-medium text-gray-600 transition-colors hover:bg-white/70 hover:text-gray-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
            aria-label={`Download ${selectedFilename}`}
            title="Download displayed document"
          >
      ...
      >
        ...
        >
          <div className={cn("shrink-0 p-4")}>
            <div className="mb-4">
              <div className="mb-3 text-xs font-medium text-gray-900">Name</div>
              {editingName ? (
            ...
            <div className="rounded-xl bg-gray-100/70 px-3 py-3">
              <div className="space-y-1.5">
                ...
                <DataRow
                  label="Pages"
                  value={
                    selectedPageCount != null ? String(selectedPageCount) : "—"
                  }
                />
              </div>
            </div>
            ...
            >
              <div className="min-h-0 flex-1 overflow-y-auto py-2">
                {versionsLoading && versions.length === 0 ? (
                  <div className="space-y-1.5">
                    {Array.from({
                      length: versionSkeletonCount(doc.active_version_number),
                    }).map((_, index) => (
                              ...
                              {deleted ? (
                                ...
                                <>
                                  ...
                                  <button
                                    ...
                                    disabled={
                                      (canDelete && activeVersionCount <= 1) ||
                                      deletingVersionId != null
                                    }
                                    className={cn(
...
function hasExtensionChange(previous: string, next: string) {
  ...
  return (
    filenameExtension(next)?.toLowerCase() !== previousExtension.toLowerCase()
  );
}
