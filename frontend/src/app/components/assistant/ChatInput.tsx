...
import type { ReasoningEffort } from "@/app/lib/mikeApi";
...
export const ChatInput = forwardRef<ChatInputHandle, Props>(function ChatInput(
  ...
  const selectedModel = modelCatalog.models.find((item) => item.id === model);
  const reasoningEfforts = (selectedModel?.reasoningEfforts ??
    []) as ReasoningEffort[];
  const [reasoningByModel, setReasoningByModel] = useState<
    Partial<Record<string, ReasoningEffort>>
  >({});
  const requestedReasoning = reasoningByModel[model];
  const reasoningEffort = reasoningEfforts.includes(
    requestedReasoning as ReasoningEffort,
  )
    ? requestedReasoning
  ...
  const handleAddDocsFromSelector = useCallback((selectedDocs: Document[]) => {
    setAttachedDocs((prev) => {
        ...
        <div className="rounded-[18px] border border-white/65 bg-white/60 shadow-[0_4px_10px_rgba(15,23,42,0.12),inset_0_1px_0_rgba(255,255,255,0.85),inset_0_-6px_14px_rgba(255,255,255,0.18)] backdrop-blur-2xl md:rounded-[22px]">
          ...
          >
            <div className="flex items-center gap-1">
              <label className="sr-only" htmlFor="chat-jurisdiction">
              ...
              {!hideWorkflowButton && (
                ...
                >
                  ...
                  <span
                    className={compactControls ? "hidden" : "hidden sm:inline"}
                  >
                    Workflows
                  </span>
                </button>
            ...
            <div className="flex items-center gap-1">
              <ModelToggle
                ...
                modelWarning={modelCatalog.warning}
                onRefreshModels={() => void modelCatalog.refresh()}
              />
              {reasoningEfforts.length > 0 && reasoningEffort && (
              ...
              {reasoningEfforts.length > 0 && reasoningEffort && (
                <select
                  ...
                  className="h-8 rounded-lg border-0 bg-transparent px-1.5 text-xs capitalize text-gray-500 outline-none hover:bg-white/55 hover:text-gray-800 focus-visible:ring-2 focus-visible:ring-blue-500"
