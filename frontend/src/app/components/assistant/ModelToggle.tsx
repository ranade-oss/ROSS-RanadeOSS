...
export function ModelToggle({
  ...
  const selected =
    models.find((m) => m.id === value) ?? MODELS.find((m) => m.id === value);
      ...
      <DropdownMenuContent className="w-56 z-50" side="top" align="end">
        ...
        {onRefreshModels && (
          <>
            <DropdownMenuSeparator />
            {modelWarning && (
              <div className="px-2 py-1 text-[11px] leading-4 text-amber-700">
                {modelWarning}
              </div>
            )}
            <DropdownMenuItem
            ...
            >
              <RefreshCw
                className={`mr-2 h-3.5 w-3.5 ${modelsLoading ? "animate-spin" : ""}`}
              />
              Refresh models
            </DropdownMenuItem>
          </>
