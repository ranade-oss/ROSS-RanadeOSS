...
export async function getUserModelSettings(
  ...
  const configuredSourceProviders = normalizeProviders(
    data?.enabled_source_providers,
    legacyUs,
  );
  const enabledSourceProviders = api_keys.canlii
    ? [...new Set([...configuredSourceProviders, "canlii-licensed"])]
    : configuredSourceProviders;
  const defaultCountry = data?.default_country === "US" ? "US" : "CA";
  const defaultProvince =
    defaultCountry === "CA" && data?.default_province !== null ? "ON" : null;
